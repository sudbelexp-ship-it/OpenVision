...
## 9. Подтверждено реальным смоук-тестом GigaChat (Фаза 3a)

`tools/gigachat_smoke.py` успешно прогнан пользователем 2026-09-11 с реальным
ключом. Модель `GigaChat-2-Max` корректно описала тестовое изображение (чашка
с паром) на русском. Ниже — точные поля ответов API, подтверждённые на
практике (без самих значений токена/ключа) — Swift-код в Фазе 3b пишется по
ним, а не по одним предположениям из документации.

### OAuth (`POST /api/v2/oauth`)
- `access_token` — строка (в этом прогоне 1233 символа; длина не
  гарантирована, не хардкодить).
- `expires_at` — **целое число, миллисекунды Unix-эпохи** (13 цифр),
  подтверждено пересчётом (`1789164972473 / 1000` → `2026-09-11 22:16:12 UTC`,
  совпадает с временем прогона + ~30 мин жизни токена). Значит в Swift это
  `Date(timeIntervalSince1970: expiresAtMs / 1000)`, **не** `expires_in`
  (секунды-длительность) — поле именно абсолютная метка времени истечения.

### `GET /api/v1/models`
- Ответ: `{"data": [{"id": "..."}, ...]}` (OpenAI-совместимый формат).
- В прогоне вернулось 14 моделей, включая: `GigaChat`, `GigaChat-2`,
  `GigaChat-2-Max`, `GigaChat-2-Pro`, `GigaChat-Max` — то есть ровно те id,
  что заданы в `PLAN.md` (`GigaChat-2-Max` для изображений, `GigaChat-2-Pro`
  для текста), опечаток в названиях моделей нет.

### `POST /api/v1/files`
- Ответ содержит `id` — **UUID-строка**, например
  `03e23836-753b-4218-8f6b-68aac90a3af5` (формат подтверждён, не числовой ID).

### `POST /api/v1/chat/completions`
- Формат ответа — OpenAI-совместимый: `choices[0].message.content` — строка
  с ответом ассистента. В прогоне: `attachments: [file_id]` в
  user-сообщении сработало (модель увидела и описала изображение) — значит
  паттерн «сначала загрузить файл через `/files`, потом сослаться на его
  `id` в `attachments`» — рабочий и единственно нужный (никакого inline
  base64, в отличие от OpenAI/Gemini — подтверждает риск №9 из раздела 8).

### `POST /api/v1/files/{id}/delete`
- **Подтверждено: метод POST, не DELETE.** Путь `/files/{id}/delete` (не
  `DELETE /files/{id}`) — так и было изначально реализовано в
  `gigachat_smoke.py` по данным официальной документации, реальный запрос
  прошёл успешно (файл удалён после использования).

### Вывод для Swift (`GigaChatClient.swift`, Фаза 3b)
1. `expires_at` — миллисекунды, абсолютная метка, не длительность.
2. Все ответы (`/models`, `/files`, `/chat/completions`) — единообразный
   OpenAI-совместимый JSON, `JSONDecoder` без экзотики.
3. Загрузка изображения — обязательно двухшаговая: `POST /files` → получить
   `id` → передать этот `id` в `attachments` чат-запроса. Удаление файла —
   `POST /files/{id}/delete` **после** получения ответа модели.
4. Сертификат `certs/russian_trusted_root_ca.pem` через кастомный
   `URLSessionDelegate` — необходимость подтверждена (без `verify=<cert>`
   в Python запросы тоже не прошли бы стандартную проверку доверия iOS/Windows
   к цепочке Минцифры).
